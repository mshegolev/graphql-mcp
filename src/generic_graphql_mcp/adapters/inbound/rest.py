...
Run: uvicorn generic_graphql_mcp.adapters.inbound.rest:app --host 0.0.0.0 --port 8000
"""
...
from generic_graphql_mcp.domain.errors import MutationGuardError, QueryDepthError, SchemaResolutionError
from generic_graphql_mcp.domain.subscription_limiter import SubscriptionRateLimiter
...
app = FastAPI(title="generic-graphql-mcp", version="0.1.0")
...
async def sse_subscribe(request: Request, query: str, variables: str | None = None) -> StreamingResponse | JSONResponse:
    ...
    async def _event_stream():
        ...
        try:
            ...
            error_payload = _json.dumps({"error": "Install generic-graphql-mcp[subscriptions]"})
...
async def websocket_subscribe(ws: WebSocket) -> None:
    ...
    try:
        ...
        await ws.send_json({"type": "error", "payload": [{"message": "Install generic-graphql-mcp[subscriptions]"}]})
        await ws.close()
    except Exception as exc:
